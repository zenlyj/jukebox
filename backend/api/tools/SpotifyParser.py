...
class SpotifyParser:
    def parse_spotify_search(
        ...
        return (
            self._get_best_result(result[tracks_key][items_key], query)
    ...
    def parse_spotify_artists(self, result_text: str) -> List[SpotifyArtistData]:
        ...
        for artist_data in artists_data:
            name = self._extract_artist_name(artist_data)
            genres = self._extract_artist_genres(artist_data)
            spotify_id = self._extract_artist_spotify_id(artist_data)
    ...
    def __extract_track_data(self, track_data: dict) -> SpotifySearchTrackData:
        name = self._extract_track_name(track_data)
        artists = self._extract_track_artists(track_data)
        uri = self._extract_track_uri(track_data)
        album_cover = self._extract_track_album_cover(track_data)
        duration = self._extract_track_duration(track_data)
        spotify_id = self._extract_track_spotify_id(track_data)
        return name, artists, uri, album_cover, duration, spotify_id

    def _extract_track_name(self, track_data: dict) -> Union[str, None]:
        name_key = "name"
    ...
    def _extract_track_artists(
        ...
        for artist_data in artists_data:
            name = self._extract_track_artist_name(artist_data)
            id = self._extract_track_artist_spotify_id(artist_data)
            res.append((name, id))
        return res

    def _extract_track_artist_name(self, artist_data: dict) -> Union[str, None]:
        artist_name_key = "name"
        return artist_data[artist_name_key] if artist_name_key in artist_data else None

    def _extract_track_artist_spotify_id(self, artist_data: dict) -> Union[str, None]:
        artist_genres_key = "id"
        return (
            artist_data[artist_genres_key] if artist_genres_key in artist_data else []
        )

    def _extract_track_uri(self, track_data: dict) -> Union[str, None]:
        uri_key = "uri"
        return track_data[uri_key] if uri_key in track_data else None

    def _extract_track_album_cover(self, track_data: dict) -> Union[str, None]:
    ...
    def _extract_track_duration(self, track_data: dict) -> Union[str, None]:
        duration_key = "duration_ms"
        return track_data[duration_key] if duration_key in track_data else None

    def _extract_track_spotify_id(self, track_data: dict) -> Union[str, None]:
        id_key = "id"
        return track_data[id_key] if id_key in track_data else None

    def _extract_artist_name(self, artist_data: dict) -> Union[str, None]:
        artist_name_key = "name"
        return artist_data[artist_name_key] if artist_name_key in artist_data else None

    def _extract_artist_genres(self, artist_data: dict) -> List[str]:
        artist_genres_key = "genres"
        return (
            artist_data[artist_genres_key] if artist_genres_key in artist_data else []
        )

    def _extract_artist_spotify_id(self, artist_data: dict) -> Union[str, None]:
