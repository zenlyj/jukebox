...
class SpotifyService:
    def create_token(self, authorization_code: str) -> SpotifyAuthorizationResponse:
        response = requests.post(
            ...
            data=self._get_token_data(authorization_code),
        )
        ...
        return self._to_spotify_authorization_response(
            access_token, refresh_token, expires_in
        )
    ...
    def refresh_token(self, refresh_token: str) -> SpotifyAuthorizationResponse:
        response = requests.post(
            TOKEN_ENDPOINT,
            headers=self._token_headers(),
            data=self._refresh_token_data(refresh_token),
        )
        if response.status_code != 200:
            self._handle_token_error(response)
        response_data = json.loads(response.text)
    ...
    def search(
        ...
        response = requests.get(
            SEARCH_ENDPOINT,
            headers=self._search_headers(access_token),
            params=self._search_params(query, query_type),
        )
        if response.status_code != 200:
    ...
    def get_artists(
        ...
        response = requests.get(
            ARTISTS_ENDPOINT,
            params=self._artist_params(artists_spotify_id),
    ...
    def _get_token_data(self, authorization_code: str) -> dict:
        return {
    ...
    def _refresh_token_data(self, refresh_token: str) -> dict:
    ...
    def _search_headers(self, access_token: str) -> dict:
        return {
    ...
    def _search_params(self, query: str, query_type: str) -> dict:
        return {"q": query, "type": query_type, "limit": 5}

    def _artist_params(self, ids: str) -> dict:
        return {"ids": ids}

    def _artist_headers(self, access_token: str) -> dict:
    ...
    def _to_spotify_authorization_response(
    ...
    def _to_search_spotify_response(
        self,
        name: str,
